...
const navItems = [
  ...
  { href: "/dashboard/templates", label: "Templates", icon: BoxIcon },
...
const adminItems = [
  { href: "/dashboard/admin", label: "Overview", icon: ShieldIcon },
  { href: "/dashboard/admin/users", label: "Users", icon: UsersIcon },
  { href: "/dashboard/admin/deployments", label: "Deployments", icon: ServerIcon },
  { href: "/dashboard/admin/resources", label: "Resources", icon: CpuIcon },
];
...
export function Sidebar() {
  ...
  return (
    <aside className="flex h-screen w-64 flex-col border-r border-mist bg-white">
      <div className="flex items-center gap-2 px-6 py-5 border-b border-mist">
            ...
            <Link
              ...
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                isActive
                  ...
                  : "text-forest-dark/60 hover:bg-mist hover:text-forest-dark"
              }`}
            >
        ...
        {isAdmin && (
          <>
            <div className="pt-4 pb-1 px-3">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-forest-dark/60">
                Admin
              </p>
            </div>
            {adminItems.map((item) => {
      ...
      <div className="border-t border-mist px-4 py-4">
        <div className="flex items-center gap-3">
          <UserButton
            appearance={{
              elements: {
                avatarBox: "h-8 w-8 ring-2 ring-mist",
                userButtonPopoverCard: "border border-mist shadow-lg",
                userButtonPopoverActionButton: "text-forest-dark hover:bg-sage",
              },
            }}
          />
          <div className="flex flex-col min-w-0">
            <span className="text-sm font-medium text-forest-dark truncate">
              {user?.firstName || user?.primaryEmailAddress?.emailAddress?.split("@")[0] || "Account"}
            </span>
            {user?.primaryEmailAddress?.emailAddress && (
              <span className="text-xs text-forest-dark/70 truncate">
                {user.primaryEmailAddress.emailAddress}
              </span>
            )}
          </div>
